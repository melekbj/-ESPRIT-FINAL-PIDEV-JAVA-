...
public class UserController implements UController<User> {
   ...
   public void AddUser(User t) {
    String requete = "insert into user(email,roles,password,nom,prenom,age,adresse,ville,image,genre,phone,etat) values(?,?,?,?,?,?,?,?,?,?,?,?)";
    try {
        ...
        pst.setString(10, t.getGenre());
        pst.setString(11, t.getPhone());
        if (t.getRoles().equals("ROLE_PARTNER")) {
            pst.setInt(12, -1); // set etat to -1 for partner users
        } else {
            pst.setInt(12, 0); // set etat to 0 for other users
        }
